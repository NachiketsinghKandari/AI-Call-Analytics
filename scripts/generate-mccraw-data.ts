/**
 * Build-time script to generate pre-computed McCraw Law data from the McCrawLaw-analysis/ folder.
 * Run with: npx tsx scripts/generate-mccraw-data.ts
 */
...
function matchTxtFile(jsonName: string, txtFileNames: Set<string>): string | null {
  // Pattern 1: Direct match (both have _gemini suffix in McCraw Law data)
...
function generate() {
  ...
  if (fs.existsSync(OUTPUT_PATH)) {
    ...
    if (stats.size > 0) {
      console.log(`McCraw Law data already exists at ${OUTPUT_PATH} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
      console.log('Skipping generation. Delete the file to regenerate.');
      return;
    }
  }

  console.log(`Reading files from ${BASE_DIR}...`);

  if (!fs.existsSync(ANALYSIS_DIR)) {
    console.log(`Analysis directory not found: ${ANALYSIS_DIR}`);
    console.log('No McCraw Law data will be generated.');
    return;
  }

  // Read JSON files from analysis/ folder
  const jsonFileNames = fs.readdirSync(ANALYSIS_DIR).filter(f => f.endsWith('.json'));
  ...
  if (jsonFileNames.length === 0) {
    console.log('No JSON files found in analysis folder.');
    return;
  }
  ...
  for (const txtName of txtFileNames) {
    try {
      txtContents.set(txtName, fs.readFileSync(path.join(TRANSCRIPTS_DIR, txtName), 'utf-8'));
    } catch (err) {
  ...
  for (const jsonName of jsonFileNames) {
    try {
      const content = fs.readFileSync(path.join(ANALYSIS_DIR, jsonName), 'utf-8');
